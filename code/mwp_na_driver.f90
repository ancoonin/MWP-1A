...
! blocks used by NA routines:
 ...
 scales(2:6)=20.0 ! scaling by fraction of estimated MWP1A GMSL rise
 ranges(1,1:5)=0.0 !min GMSL equivalent contribution from each ice sheet
 
  ! values are from prem.inc file located in /code/include directory:
 ...
 scales(7:11)=650.0 ! range of potential start times ice melting
 ...
 scales(12:16)=500.0   ! range of potential durations of 
                       ! ice melting < 500 years)
                       
 ranges(1,11:15)=25.0  ! if there is nonzero melting,
                       ! the shortest duration is 1 timestep
...
subroutine forward(nd,model,lppd,model_overwrite)
  ...
  ! note that we consider 5 potential MWP-1A sources
  real, dimension(5) :: per_tstep ! amount of ice lost per timestep
  real, dimension(5) :: f_ice     ! multiplicative factor to shave off a
  ...
  integer, dimension(5) :: start_ix, end_ix, nactivetimesteps ! for each ice source
  real, dimension(5) :: start_time, end_time, dur ! for each ice source
...
! account for any other ice that falls outside of the 5 potential MWP-1A ice sources
! when constructing the ice load at each timestep, to ensure that this mass is not 
! artificially lost during MWP1a: 

baseload(:,:)=preMWP_icegrid(:,:) - LIS_preMWP(:,:) - GIS_preMWP(:,:) &
                - EIS_preMWP(:,:) - WAIS_preMWP(:,:) - WL_preMWP(:,:)
...
contribution(:,:)= LIS_preMWP(:,:)*per_tstep(i)*f_ice(i) ! spatially varying ice thickness to shave off 
          ...
          do j=1,ntimesduringMWP
                ...
                write(*,*) 'something wrong with the melting time discretization for LIS'
                endif
          enddo
...
per_tstep(i)=model_overwrite(i)/real(nactivetimesteps(i))
contribution(:,:)=GIS_preMWP(:,:)*per_tstep(i)*f_ice(i)
          ...
          do j=1,ntimesduringMWP
                ...
                write(*,*) 'something wrong with the melting time discretization for GIS'
                endif
          enddo
...
nactivetimesteps(i)=NINT(dur(i)/25.0) 
per_tstep(i)=model_overwrite(i)/real(nactivetimesteps(i)) 
contribution(:,:)=EIS_preMWP(:,:)*per_tstep(i)*f_ice(i)
...
nactivetimesteps(i)=NINT(dur(i)/25.0) 
per_tstep(i)=model_overwrite(i)/real(nactivetimesteps(i)) 
contribution(:,:)=WL_preMWP(:,:)*per_tstep(i)*f_ice(i)
...
! MWP1A RSL from SLIP data (located in mwp_param.inc)
...
! (e.g., assuming a constant slope of sea-level rise over a
! standard time window across all sites), we only consider
! misfit with the observed data before and after the data
! gap at each site
...
! Coonin 2022                                                            !
!                                                                        !
! writes out the final model to the file assigned logical unit lu        !
!                                                                        !
!       Note: Calls no other routines                                    !
! ====================================================================== !
