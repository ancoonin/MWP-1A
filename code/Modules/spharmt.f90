...
! Modified by Harriet Lau in 2023. see HL tag for changes
...
! --> HL
REAL, DIMENSION(spheredat%nlons, spheredat%nlats) :: Ztest             !spatial field
! <-- HL
Zi(:,:) = Z(:,:)

!! HL add
do n=1,spheredat%nlats
  do m=1,spheredat%nlons
     Ztest(m,n) = Z(n,m)
ENDDO
ENDDO
...
CALL spharm(spheredat,Ztest,Uvec,1) !! HL added
...
CALL spharm(spheredat,Ztest,Uvec,-1)
